...
def load_meshes(path1="lion", path2="cat",n_poses=9):
    '''
    Load each mesh and store in an array
    '''
    global num_patches
    meshes = []
    rest = trimesh.load_mesh(os.path.join(path1,f"{path1}-reference.obj"))
    rest.merge_vertices(merge_tex=True, merge_norm=True)
    for i in range(1,n_poses+1):
        pose = trimesh.load_mesh(os.path.join(path1,f"{path1}-{i:02}.obj"))
    ...
    meshes_2 = []
    rest = trimesh.load_mesh(os.path.join(path2,f"{path2}-reference.obj"))
    rest.merge_vertices(merge_tex=True, merge_norm=True)
    for i in range(1,n_poses+1):
        pose = trimesh.load_mesh(os.path.join(path2,f"{path2}-{i:02}.obj"))
        pose.merge_vertices(merge_tex=True, merge_norm=True)
...
def run_interpolation():
    ...
    rest = trimesh.load_mesh("head-poses/head-reference.obj")
    rest.merge_vertices(merge_tex=True, merge_norm=True)
    print(len(rest.vertices))
    pose = trimesh.load_mesh("head-poses/head-04-grin.obj")
    pose.merge_vertices(merge_tex=True, merge_norm=True)
    meshes.append(pose)
    print(len(pose.vertices))
    pose = trimesh.load_mesh("head-poses/head-02-cry.obj")
    pose.merge_vertices(merge_tex=True, merge_norm=True)
    meshes.append(pose)
    print(len(pose.vertices))
    """
...
def encode(rest_pose, current_pose, patches):
    ...
    Q_matrices = np.zeros((len(patches), 3, 3))
    S_matrices = np.zeros((len(patches), 3, 3))
    Q_bar = np.zeros((3, 3))
    v_bar = np.zeros(3)
    for vert in current_pose.vertices:
        v_bar += vert
    v_bar = v_bar / len(patches)
    sum_q_i = np.zeros((num_patches, 3, 3))
    ...
    for i in range(len(patches)):
        ...
        Q_matrices[i] = Q
        S_matrices[i] = S
    Q_bar, _ = scipy.linalg.polar(Q_bar)
    Q_bar = rot2eul(Q_bar)
    Q_bar = Q_bar.flatten()
    G_matrices = np.zeros((num_patches, 3, 3))
    for i in range(num_patches):
        G_matrices[i] = sum_q_i[i] / num_in_patch[i]
    ...
    for ind, adj in enumerate(patch_adj):
        ...
        G_inv_G = rot2eul(scipy.linalg.polar(G_inv_G)[0])
    ...
    for ind, Q in enumerate(Q_matrices):
        patch = patches[ind]
        G = G_matrices[patch]
        G_inv_Q[ind] = rot2eul(scipy.linalg.polar(G@Q)[0])
...
def anim_interp(coord_1, coord_2, patch_adj, patch_map, patches, rest, obj, time_per_frame, num_frames, start=0):
    for frame in range(0, num_frames + 1):  # start from 1 because 0 is the Basis

        # This is where you would call your Python function to get the vertex data at this time step
        # vertices = your_function(frame)
        print(frame+start)
        coord = interpolation([coord_1, coord_2], [1-frame/num_frames, frame/num_frames])
        vertices = decode(coord, patch_adj, patch_map,
                          patches, copy.deepcopy(rest))
        ...
        for vertex in obj.data.vertices:
            vertex.keyframe_insert('co', frame=start*time_per_frame+frame*time_per_frame)

#load_meshes()
#run_interpolation()
def make_animation(start, *paths):
    global num_patches
    meshes = []
    ...
    for i in range(len(meshes)):
        coord, G_matrices, patch_adj, patches, patch_map = encode(
        rest, meshes[i], patches)
        coords.append(coord)
    coord = interpolation([coords[0], coords[1]], [0, 1])
    vertices = decode(coord, patch_adj, patch_map,
                          patches, copy.deepcopy(rest))
    out_mesh = copy.deepcopy(rest)
    out_mesh.vertices = vertices
...
# Set location and scene of object
    ...
    mesh.from_pydata(out_mesh.vertices, [], out_mesh.faces)
    mesh.update(calc_edges=True)
    #shape_key_basis = obj.shape_key_add(name='Basis')
    num_frames = 10
    print("Beginning animation")
    for i in range(len(meshes)-1):
        anim_interp(coords[i], coords[i+1], patch_adj, patch_map, patches, rest, obj, time_per_frame, num_frames, (num_frames+1)*i)
    anim_interp(coords[-1], coords[0], patch_adj, patch_map, patches, rest, obj, time_per_frame, num_frames, (num_frames+1)*(len(meshes)-1))
    #anim_interp(coord_2, coord_1, patch_adj, patch_map, patches, rest, obj, time_per_frame, num_frames,10)
    #anim_interp(coord_1, coord_2, patch_adj, patch_map, patches, rest, obj, time_per_frame, num_frames,20)



make_animation(0, "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-01-anger.obj", "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-02-cry.obj", "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-03-fury.obj", "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-04-grin.obj", "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-05-laugh.obj")
#make_animation(0, "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-01-anger.obj", "/Users/lucafonstad/Downloads/final-project 3/head-poses/head-02-cry.obj")
